            ...
            <mat-card class="oferta-empresa-card modern-card">
              ...
              <mat-card-content class="card-content">
                <div class="salary-info">
                  <span class="salary-badge">{{ oferta.pagoAprox }}</span>
                </div>

                <p class="job-description">{{ getDescripcionCorta(oferta.descripcion) }}</p>

                <!-- Atributos/Skills -->
                <div class="tech-stack" *ngIf="oferta.atributos && oferta.atributos.length > 0">
                  @for (atributo of getAtributosLimitados(oferta.atributos); track atributo) {
                    <mat-chip class="tech-chip" selected>{{ atributo }}</mat-chip>
                  }
                  @if (oferta.atributos.length > 4) {
                    <span class="more-tech">+{{ oferta.atributos.length - 4 }} más</span>
                  }
                </div>
              </mat-card-content>
